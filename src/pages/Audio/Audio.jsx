...
const Audio = () =>{
  ...
  return(
      <div className="kidHolder">
        <h3 className="kidTitle">аудиокниги</h3>
      ...
      {item.map(
          (item) =>
            item.type === "audio" &&  item.isDiscount === true && value[0] <= item.price && value[1] >= item.price &&
            <DiscountCard
            type = "simple"
            name={item.name}
            author={item.author}
        ...
        {item.map(
          (item) =>
            item.type === "audio" &&  item.isDiscount === false && value[0] <= item.price && value[1] >= item.price &&
            <Card
            type = "simple"
              name={item.name}
              author={item.author}
