...
const Fantasy = () =>{
  ...
  const booksType = books.filter((i) => i.type === "fantasy");
  ...
  return(
      ...
      {item.map(
            (item) => {
              if (item.type === "fantasy" &&  item.isDiscount === true && value[0] <= item.price && value[1] >= item.price) {
                return <DiscountCard
                type = "simple"
                name={item.name}
                author={item.author}
              ...
              else if (item.type === "fantasy" &&  item.isDiscount === false && value[0] <= item.price && value[1] >= item.price) {
                return <Card
                   type = "simple"
                   name={item.name}
                   author={item.author}
