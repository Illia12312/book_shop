...
const Novel = () =>{
  ...
  const booksType = books.filter((i) => i.type === "novel");
  ...
  return(
      <div className="kidHolder">
        <h3 className="kidTitle">Романы</h3>
      ...
      {item.map(
            (item) => {
              if (item.type === "novel" &&  item.isDiscount === true && value[0] <= item.price && value[1] >= item.price) {
                return <DiscountCard
                type = "simple"
                name={item.name}
                author={item.author}
              ...
              else if (item.type === "novel" &&  item.isDiscount === false && value[0] <= item.price && value[1] >= item.price) {
                return <Card
                   type = "simple"
                   name={item.name}
                   author={item.author}
