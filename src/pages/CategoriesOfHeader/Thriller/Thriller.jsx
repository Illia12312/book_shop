...
const Thriller = () =>{
  ...
  const booksType = books.filter((i) => i.type === "thriller");
  ...
  return(
      ...
      {item.map(
            (item) => {
              if (item.type === "thriller" &&  item.isDiscount === true && value[0] <= item.price && value[1] >= item.price) {
                return <DiscountCard
                type = "simple"
                name={item.name}
                author={item.author}
              ...
              else if (item.type === "thriller" &&  item.isDiscount === false && value[0] <= item.price && value[1] >= item.price) {
                return <Card
                   type = "simple"
                   name={item.name}
                   author={item.author}
